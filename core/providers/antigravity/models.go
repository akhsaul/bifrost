...
// This is a STATIC FALLBACK only — the authoritative list is always fetched
// dynamically from /v1internal:fetchAvailableModels (see FetchAvailableModelsFromAPI).
// The IDs below mirror a real fetchAvailableModels response; keep them in sync
// when the server list changes (verify with the live endpoint).
var AntigravityPublicModels = []schemas.Model{
	...
	{ID: "gemini-2.5-flash", Name: schemas.Ptr("Gemini 3.1 Flash Lite"), ContextLength: schemas.Ptr(1048576), MaxInputTokens: schemas.Ptr(1048576), MaxOutputTokens: schemas.Ptr(65535)},
	{ID: "gemini-2.5-flash-lite", Name: schemas.Ptr("Gemini 3.1 Flash Lite"), ContextLength: schemas.Ptr(1048576), MaxInputTokens: schemas.Ptr(1048576), MaxOutputTokens: schemas.Ptr(65535)},
	{ID: "gemini-2.5-flash-thinking", Name: schemas.Ptr("Gemini 3.1 Flash Lite"), ContextLength: schemas.Ptr(1048576), MaxInputTokens: schemas.Ptr(1048576), MaxOutputTokens: schemas.Ptr(65535)},
	...
	{ID: "gemini-3.1-flash-image", Name: schemas.Ptr("Gemini 3.1 Flash Image")},
	...
	{ID: "gemini-3.6-flash-tiered", ContextLength: schemas.Ptr(1048576), MaxInputTokens: schemas.Ptr(1048576), MaxOutputTokens: schemas.Ptr(65536)},
	...
	{ID: "gemini-3.7-flash-tiered", ContextLength: schemas.Ptr(1048576), MaxInputTokens: schemas.Ptr(1048576), MaxOutputTokens: schemas.Ptr(65536)},
	{ID: "gemini-3.8-flash-high", Name: schemas.Ptr("Gemini 3.8 Flash (High)"), ContextLength: schemas.Ptr(1048576), MaxInputTokens: schemas.Ptr(1048576), MaxOutputTokens: schemas.Ptr(65536)},
	{ID: "gemini-3.8-flash-low", Name: schemas.Ptr("Gemini 3.8 Flash (Low)"), ContextLength: schemas.Ptr(1048576), MaxInputTokens: schemas.Ptr(1048576), MaxOutputTokens: schemas.Ptr(65536)},
	{ID: "gemini-3.8-flash-medium", Name: schemas.Ptr("Gemini 3.8 Flash (Medium)"), ContextLength: schemas.Ptr(1048576), MaxInputTokens: schemas.Ptr(1048576), MaxOutputTokens: schemas.Ptr(65536)},
...
// AntigravityModelAliases maps CONVENIENCE names that the server does NOT
// recognize to real upstream model IDs. Keys must never be valid server model
// IDs — rewriting a model the server accepts corrupts the request (that was
// the "tiered(high)" bug). The server's own model list is authoritative and
// changes without notice; valid IDs must always pass through unchanged.
...
// ResolveModel resolves convenience aliases and strips any routing prefix.
// Server-recognized model IDs are returned unchanged.
