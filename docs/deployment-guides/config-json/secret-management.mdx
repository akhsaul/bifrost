...
Configure Doppler SecretOps to resolve secrets via the Doppler API v3.
...
| Field | Required | Description |
|-------|----------|-------------|
| `token` | Yes | Doppler API token (Service Token `dp.st.*`, Service Account Token `dp.sa.*`, Personal Token, or CLI token). |
| `project` | No | Default Doppler project name or slug (optional if specified in reference path or using a Service Token). |
| `config` | No | Default Doppler config / environment name (e.g. `prd`, `stg`, `dev`). |
| `base_url` | No | Doppler API endpoint. Defaults to `https://api.doppler.com`. |
| `timeout_sec` | No | HTTP request timeout in seconds (default: `10`). |
...
| `type` | Yes | Backend: `aws-secrets-manager`, `gcp-secret-manager`, `hashicorp-vault`, or `doppler`. |
